	...
	switch (a)
	 {
		case 1:  // crop and resize
			{
				...
				int width , height ;
				cout<< «Enter the number of columns and rows » <<endl ;  						cin>>width>>height ;
				...
				blur(InputArray dst, OutputArray final, Size ksize, Point 								anchor=Point(-1,-1),int borderType=BOR	DER_DEFAULT ) ;        													// smoothen the new image
		...
		case 2: //filter
  			{	
				...
				cout<< «Enter the number of columns and rows » <<endl ;  									cin>>width>>height ;
				Size ksize (width , height);
				double maxValue , c  ;																int adaptiveMethod , thresholdType , blockSize ;
